...
public class GameMod : MonoBehaviour {
    ...
    {
        //clear old food
        Transform[] placedFood = parent.GetComponentsInChildren<Transform>();
        for (int i = 0; i < placedFood.Length; i++)
        {
            if(placedFood[i] != null && placedFood[i].gameObject != parent)
                DestroyImmediate(placedFood[i].gameObject);
        }
    }
